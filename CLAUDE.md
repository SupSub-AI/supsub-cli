...
| 我的关注点 / 关注点列表 / **我关注了哪些** / 看某个关注点里的内容（文章）/ 关注点有哪些未读 / 删除关注点 | `supsub-focus` | `supsub focus list` / `contents` / `remove`（**无新增**；删除不可逆） |
...
> **订阅源（sub）vs 关注点（focus）** —— 二者都是「我在追的东西」，但不同：**订阅源**是具体的一个号（公众号 / 网站 / 推特账号），按 `sourceId`+`sourceType` 操作；**关注点**是用户自定义的主题/关注方向（带 AI 聚合，列表项形如 `{id, icon, title}`，如「Agent 行业落地」），按 `focus id` 操作。**判别动词**：「订阅 / 订阅了」→ `supsub-sub`（订阅源）；「关注 / 关注了 / 关注点」→ `supsub-focus`（关注点）。即「我订阅了哪些」→ `supsub-sub`；而「我关注了哪些」「我的关注点」「这个关注点里有什么」→ `supsub-focus`。
>
...
>
> **🔍 搜索范围判定（全局 vs 我的范围）** —— 收到「搜 / 查 / 找内容」类请求时，先判断范围：
> 1. **未指明范围 → 全局搜索 `supsub search`（全站）。** 「搜一下大模型」「在 supsub 里搜 RAG」「查 X 相关文章」都属此类。
> 2. **明确限定到本人数据范围 → 属确定范围，不走全局搜索。** 出现「我的订阅」「我的公众号」「我的网站」「我的关注点」「我订阅的…」等限定词时：「我的订阅 / 公众号 / 网站」→ `supsub-sub`；「我的关注点」→ `supsub-focus`。
> 3. 注意：CLI 目前没有「在我的某范围内按关键词全文搜」的命令——确定范围的请求落到对应 skill 的**列表 / 内容浏览**命令（`sub list`/`sub contents`/`focus list`/`focus contents`），全文关键词搜索只有全局 `supsub search` 一条路径。
